  ...
  for (var i = 1; i < timings.length; i++){
    ...
    case "runCode":
      pop.code({
        start: start,
        onStart: function(options){
          var curSlide_ = "#slide-" + window.slidedeck.curSlide_;
          var codeCell = $(curSlide_).find(".interactive").data('cell')
          runCode(codeCell)  
        }
      });
      break;
...
function findSlideTiming(i, timings){
  var j = i;
  while (timings[j].slide != i){
    j = j + 1
  }
  return(j)
}
